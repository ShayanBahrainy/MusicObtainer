...
def process_song(track_id: str) -> None:
    ...
    try:
        ...
        log.info(f"  Format: {actual_format}")

        tmp_path = _download_with_retry(track_url)
        log.info(f"  Downloaded → {tmp_path}")
        ...
        log.info(f"  Tagged: {artist} – {title}")
...
async def response(flow: http.HTTPFlow) -> None:
    ...
    if "/api.json/0.2/file/url?" not in url:
        return

    track_id = get_track_id_from_url(url)
    if not track_id:
        log.warning(f"Could not parse track_id from: {url}")
        return

    await asyncio.to_thread(process_song, track_id)